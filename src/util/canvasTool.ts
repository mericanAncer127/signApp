...
import { v4 } from "uuid";
import { RECT_TOOL_SIZE } from "../constants";
...
export const drawFirstImage = (
  initImageDatas: InitCanvasImageData[],
  curPdfCanvas: HTMLCanvasElement | null
) => {
  if (!initImageDatas.length) return;
  if (curPdfCanvas) {
    const drawCanvas = initImageDatas.filter(
      (images) => images.pdfCanvas === curPdfCanvas
    );
    ...
    ctx.clearRect(
      0,
      0,
      drawCanvas[0].pdfCanvas.width,
      drawCanvas[0].pdfCanvas.height
    );
